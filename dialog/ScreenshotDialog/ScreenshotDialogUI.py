...
class ScreenshotDialogUI(wx.Dialog):
	"""docstring for ScreenshotDialogUI"""
	def __init__(self, parent, id = -1, curPath = "", viewCtr = None, params = {}):
		self.initParams(params);
		super(ScreenshotDialogUI, self).__init__(parent, id, title = self.__params["title"], pos = self.__params.get("pos", (0,0)), size = self.__params["size"], style = self.__params["style"]);
		self._className_ = ScreenshotDialogUI.__name__;
	...
	def onRightUp(self, event):
		...
		if rect.Contains(event.GetPosition()):
			self.getCtr().onPopupMenu(event.GetPosition());
		else:
